    ...
    p <- switch(type,
      ...
      meso = {
        ...
        df_pi <- purrr::map_dfc(
          ...
          function(m) self$pim[[m]][[1]][oRow]
        )
        df_rho <- purrr::map_dfc(
          seq_along(self$net_id),
          function(m) self$pim[[m]][[2]][oCol]
        )
      ...
      "block" = {
        as.matrix(self$A[[net_id]])[
          ...
          reshape2::melt() %>%
          ggplot2::ggplot(ggplot2::aes(x = rev(Var2), y = Var1, fill = value)) +
          ggplot2::geom_tile(show.legend = FALSE) +
          ggplot2::geom_hline(
            yintercept = cumsum(tabulate(self$Z[[net_id]][[1]])[1:(self$Q[1] - 1)]) + .5,
            col = "red", size = .5
          ) +
          ggplot2::geom_vline(
            xintercept = cumsum(tabulate(self$Z[[net_id]][[2]])[(self$Q[2]):2]) + .5,
            col = "red", size = .5
          ) +
          ggplot2::scale_fill_gradient(low = "white", high = "black") +
          ggplot2::ylab("") +
          ...
          ggplot2::scale_x_discrete(
            # limits = rev, # TODO fix to have consistent display with sbm
            breaks = ""
          ) +
          # ggplot2::scale_y_reverse() +
          ggplot2::scale_y_discrete( # limits = rev,
