...
for (i in 1:iter_max) {
    ...
    fitColSepLBM.init_resultARI <- mutate(as.data.frame(do.call("rbind", lapply(
        invisible(seq.int(M)),
        function(m) {
            ARIrow <- aricode::ARI(
                bipartite_collection[[m]]$row_clustering,
...
if (verbose) cat("\nSepLBM clustering :\n - Real alpha : ", alpha, "\n - SepLBM alpha : ", fitColSepLBM$alpha, "\n")
    ...
    # Checking the clustering obtained
    fitColSpectral.init_resultARI <- mutate(as.data.frame(do.call("rbind", lapply(
        invisible(seq.int(M)),
        function(m) {
            ARIrow <- aricode::ARI(
                bipartite_collection[[m]]$row_clustering,
...
if (verbose) {
    ...
    # Checking the clustering obtained
    fitColHCA.init_resultARI <- mutate(as.data.frame(do.call("rbind", lapply(
        invisible(seq.int(M)),
        function(m) {
            ARIrow <- aricode::ARI(
                bipartite_collection[[m]]$row_clustering,
...
if (verbose) print(table.HCA.init)
    ...
    fitColHCA$optimize()
    fitColHCA.end_resultARI <- mutate(as.data.frame(do.call("rbind", lapply(
        invisible(seq.int(M)),
        function(m) {
